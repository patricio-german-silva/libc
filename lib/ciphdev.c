...
uint8_t ciphdev_create (_ciphdev *cd, uint8_t dev, uint32_t bs, const char *user_key, uint8_t len){
	cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev CREATE device\0", &dev, 1);

	// Inicializo el device
	if(cd->func_dev_initialize(cd->dev) != 0){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev CREATE IOERROR device\0", &dev, 1);
		return 1;
	}
	...
	if((secsize/_CIPHDEV_SECTOR_SIZE)*seccount >= bs){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: VOLUME SIZE (sectors)\0", (uint8_t*)&bs, 4);
		// Create speck keys
		for(uint8_t i = 0; i < 4; i++){
			cd->func_random(&(cd->speck_key1[i]));
			cd->func_random(&(cd->speck_key2[i]));
		}
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: speck key 1 (decrypt)\0", cd->u8speck_key1, 16);
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: speck key 2 (decrypt)\0", cd->u8speck_key2, 16);
		...
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: key_map\0", cd->u8key_map, 16);
		...
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: BLOCK SIZE (sectors)\0", (uint8_t*)&cd->buff_u32[126], 4);
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: VERSION\0", (uint8_t*)&cd->buff_u32[127], 4);
    ...
    // Agrego user_data[1,2]
		...
		md5_update(&md5ctx, &(cd->buff_u8[504]), 8);
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: USER DATA\0", &cd->u8user_data[0], 12);
    ...
    // randomizo 152 bytes y los cifro
		...
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: pad signature\0", &(cd->buff_u8[496]), 16);

		// write to device inmediatly
		if(cd->func_dev_write(cd->dev, cd->buff_u8, 0, 1) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev CREATE WRITE_ERROR sector 0 device \0", &dev, 1);
			return 3;
		}

		// Fill with random, if configured
		#ifdef _CIPHDEV_RANDOMIZE_UNUSED
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev CREATE randomize unused sectors on device\0", &dev, 1);
		for(uint32_t i = 1; i < bs ; i++){
			...
			if(cd->func_dev_write(cd->dev, cd->buff_u8, i, 1) != 0){
				cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev CREATE WRITE_ERROR sector\0", (uint8_t*)&bs, 1);
				return 3;
			}
		}
		#endif
		...
		if(cd->func_dev_ioctl(cd->dev, _CIPHDEV_CTRL_SYNC, cd->buff_u32) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev CREATE: SYNC CALL ERROR, device\0", &dev, 1);
			return 4;
		}
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev CREATE SUCCESS on device\0", &dev, 1);

		return 0;
	}
	cd->func_debug(_CIPHDEV_LOG_LEVEL_WARN, "Ciphdev CREATE No space left on device \0", &dev, 1);
	return 2;
}
...
uint8_t ciphdev_addkey (_ciphdev *cd, const char *user_key, uint8_t len, uint8_t index){
	if(cd->status == _CIPHDEV_STATUS_INIT){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev ADD_KEY on index\0",&index , 1);
		if(index > 9){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev ADD_KEY: Slot index out of bonds. index\0",&index , 1);
			return 4;
		}
		...
		md5_update(&md5ctx, (uint8_t*)user_key, len);
		md5_finalize(&md5ctx);  // hash on md5ctx.digest
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev ADD_KEY: user key hash\0", md5ctx.digest, 16);
		...
		// write to device inmediatly
		if(cd->func_dev_write(cd->dev, cd->buff_u8, 0, 1) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev ADD_KEY WRITE_ERROR sector 0\0", 0, 0);
			cd->status = _CIPHDEV_STATUS_WRITE_ERROR;
			return 5;
		}
		if(cd->func_dev_ioctl(cd->dev, _CIPHDEV_CTRL_SYNC, cd->buff_u32) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev ADD_KEY: SYNC CALL ERROR\0", 0, 0);
			cd->status = _CIPHDEV_STATUS_IOERROR;
			return 6;
		}
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev ADD_KEY SUCCESS on index\0",&index , 1);
		return 0;
	}
	cd->func_debug(_CIPHDEV_LOG_LEVEL_WARN, "Ciphdev ADD_KEY FAILURE on index\0",&index , 1);
	return 1;
}
...
uint8_t ciphdev_deletekey (_ciphdev *cd, uint8_t index){
	if(cd->status == _CIPHDEV_STATUS_INIT){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev DELETE_KEY on index\0",&index , 1);
		if(index > 9){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev DELETE_KEY: Slot index out of bonds. index\0",&index , 1);
			return 4;
		}
		...
		// write to device inmediatly
		if(cd->func_dev_write(cd->dev, cd->buff_u8, 0, 1) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev DELETE_KEY WRITE_ERROR sector 0\0", 0, 0);
			cd->status = _CIPHDEV_STATUS_WRITE_ERROR;
			return 5;
		}
		if(cd->func_dev_ioctl(cd->dev, _CIPHDEV_CTRL_SYNC, cd->buff_u32) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev DELETE_KEY: SYNC CALL ERROR\0", 0, 0);
			cd->status = _CIPHDEV_STATUS_IOERROR;
			return 6;
		}
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev DELETE_KEY SUCCESS on index\0",&index , 1);
		return 0;
	}
	cd->func_debug(_CIPHDEV_LOG_LEVEL_WARN, "Ciphdev DELETE_KEY FAILURE on index\0",&index , 1);
	return 1;
}
...
uint8_t ciphdev_initialize (_ciphdev *cd, uint8_t dev, const char *user_key, uint8_t len){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev INITIALIZE device\0", &dev, 1);
		// prepare md5 for user key hash
		_md5_context md5ctx;
		_speck sp;
		...
		// Inicializo el device
		if(cd->func_dev_initialize(cd->dev) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev INITIALIZE IOERROR device\0", &dev, 1);
			cd->status = _CIPHDEV_STATUS_IOERROR;
			return cd->status;
		}
		...
		if(cd->func_dev_read(cd->dev, cd->buff_u8, 0, 1) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev INITIALIZE READ_ERROR sector 0 device\0", &dev, 1);
		...
		for(uint8_t index = 0; index < 10; index++){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: test slot\0", &index, 1);
			md5_init(&md5ctx);
			md5_update(&md5ctx, (uint8_t*)user_key, len);
			md5_finalize(&md5ctx);  // hash on md5ctx.digest
			cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: user key hash\0", md5ctx.digest, 16);
			...
			cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: speck key 1 (decrypt)\0", cd->u8speck_key1, 16);
			cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: speck key 2 (decrypt)\0", cd->u8speck_key2, 16);

			if(ciphdev_header_verify(cd) == 0){
				cd->func_debug(_CIPHDEV_LOG_LEVEL_INFO, "Ciphdev INITIALIZE: SUCCESS on slot\0", &index, 1);
				// Initialize struct
        ...
        speck_decrypt(&sp, &(cd->buff_u32[80]), &(cd->buff_u32[126]));
				...
				cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: BLOCK SIZE (sectors)\0", (uint8_t*)&cd->block_size, 4);
				cd->version = cd->buff_u32[127];
				cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: VERSION", (uint8_t*)&cd->version, 4);

        speck_decrypt(&sp, &(cd->buff_u32[82]), &(cd->buff_u32[126]));
				cd->datetime = cd->buff_u32[126];
				cd->user_data[0] = cd->buff_u32[127];
				cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: DATETIME\0", (uint8_t*)&cd->datetime, 4);
		    ...
		    cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev CREATE: USER DATA\0", &cd->u8user_data[0], 12);
				...
				for(uint8_t i = 0; i < 16; i++)
					cd->u8key_map[i] = md5ctx.digest[i];
				cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: key_map\0", cd->u8key_map, 16);
				return cd->status;  // all OK
			}else{
				cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev INITIALIZE: FAILURE on slot\0", &index, 1);
			}
		}
...
uint8_t ciphdev_read (_ciphdev *cd, uint8_t* buff, uint32_t sector, uint32_t count){
	if(cd->status == _CIPHDEV_STATUS_INIT){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev READ: sector\0", (uint8_t*)&sector, 4);
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev READ: count\0", (uint8_t*)&count, 4);
		if(sector+count >= cd->block_size){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev READ: Out of bounds\0", 0, 0);
			return 2;
		}
		...
		for(uint32_t i = 0; i < count; i++){
			...
			if(cd->func_dev_read(cd->dev, cd->buff_u8, cursect, 1) != 0){
				cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev READ: READ_ERROR, VOLUME sector\0", (uint8_t*)&cursect, 4);
				cd->status = _CIPHDEV_STATUS_READ_ERROR;
				return 3;
			}
	...
	cd->func_debug(_CIPHDEV_LOG_LEVEL_WARN, "Ciphdev READ: BLOCK not initialized\0", 0, 0);
	return 1;
}
...
uint8_t ciphdev_write (_ciphdev *cd, const uint8_t *buff, uint32_t sector, uint32_t count){
	if(cd->status == _CIPHDEV_STATUS_INIT){
		...
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev WRITE: sector\0", (uint8_t*)&sector, 4);
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev WRITE: count\0", (uint8_t*)&count, 4);

		if(sector+count >= cd->block_size){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev WRITE: Out of bounds\0", 0, 0);
			return 2;
		}
		...
		for(uint32_t i = 0; i < count; i++){
			...
			if(cd->func_dev_write(cd->dev, cd->buff_u8, cursect, 1) != 0){
				cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev WRITE: WRITE_ERROR, sector\0", (uint8_t*)&cursect, 4);
				cd->status = _CIPHDEV_STATUS_WRITE_ERROR;
				return 3;
			}
		}
	...
	cd->func_debug(_CIPHDEV_LOG_LEVEL_WARN, "Ciphdev WRITE: BLOCK not initialized\0", 0, 0);
	return 1;
}
...
static uint8_t ciphdev_header_verify(_ciphdev *cd){
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev HEADER VERIFY\0", (uint8_t*)0, 0);
		_md5_context md5ctx;
		_speck sp;
		uint8_t tmp[8];
		...
		// Read sector 0
		if(cd->func_dev_read(cd->dev, cd->buff_u8, 0, 1) != 0){
			cd->func_debug(_CIPHDEV_LOG_LEVEL_ERROR, "Ciphdev HEADER VERIFY READ_ERROR sector 0\0", (uint8_t*)0, 0);
			cd->status = _CIPHDEV_STATUS_READ_ERROR;
			return cd->status;
		}
		...
		md5_finalize(&md5ctx);  // hash on md5ctx.digest
		cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev HEADER VERIFY: pad signature\0", md5ctx.digest, 16);

		// Los ultimos 4 bytes son el md5 de la randomización sin cifrar
		for(uint8_t i = 0; i < 16; i++)
			if(cd->buff_u8[i+496] != md5ctx.digest[i]){
				cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev HEADER VERIFY FAILURE\0", (uint8_t*)0, 0);
				return 1;
			}
	cd->func_debug(_CIPHDEV_LOG_LEVEL_DEBUG, "Ciphdev HEADER VERIFY SUCCESS\0", (uint8_t*)0, 0);
	return 0;
}
