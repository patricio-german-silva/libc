...
#define SHOW_INFO_LABEL_PERIOD 1000
#define SHOW_INFO_DATA_PERIOD 3000
...
#define DASH_ID 37
#define PERIOD_ID 38
#define ASTERISK_ID 39
#define UNDERSCORE_ID 40
...
typedef void (*sseg_set_gpio_def)(uint32_t port, uint32_t pin);


typedef struct{
	uint32_t segments[8][2]; //, PORT, PIN, format  G F E D C B A DP
	uint32_t digits[NUMBER_OF_DIGITS][2];
	char info_labels[NUMBER_OF_INFO_LABELS][NUMBER_OF_DIGITS];
	uint8_t hold_info;
	uint8_t curr_digit;
...
void sseg_write_info(_sseg *s, int32_t n, uint8_t l);	// Label id and number to show
