...
void ssd1306_i2c_init(_ssd1306_i2c *ssd, uint16_t address, uint8_t cols, uint8_t rows){
	...
	ssd->dma_buffer[ssd->dma_bf_iw++] = 0xFF;	// MAX contrast
...
uint8_t ssd1306_i2c_set_conntrast(_ssd1306_i2c *ssd, uint8_t conntrast){
    ...
    ssd->dma_buffer[ssd->dma_bf_iw++] = _SSD1306_CONTROL_SINGLE_BYTE_COMMAND;
	ssd->dma_buffer[ssd->dma_bf_iw++] = _SSD1306_COMMAND_SET_CONNTRAST;
	ssd->dma_buffer[ssd->dma_bf_iw++] = conntrast;
	return 0;
}
...
void ssd1306_i2c_work(_ssd1306_i2c *ssd, uint8_t period){
	...
	if(ssd->TX_errors > _SSD1306_DMA_I2C_TX_MAX_ERRORS)	return;
	...
	// dma_status utilizado para asignar tiempo de espera - esto hay que analoizarlo
	if(ssd->dma_status > 0){
		ssd->dma_status--;
		return;
	}
	...
	if(ssd->display_update != 0){
		if(ssd->display_update > 4){	// Primero debo asegurar los indices del display el principio de la grilla
			switch(ssd->mode){
